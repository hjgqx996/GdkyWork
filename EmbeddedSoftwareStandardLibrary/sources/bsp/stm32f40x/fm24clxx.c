...
#define GPIO_PORT_WP	GPIOD

#define GPIO_PIN_SDA	GPIO_Pin_7
#define GPIO_PIN_CLK	GPIO_Pin_6
#define GPIO_PIN_WP 	GPIO_Pin_7

#define GPIO_CLK_SDA	RCC_AHB1Periph_GPIOB
#define GPIO_CLK_CLK	RCC_AHB1Periph_GPIOB
#define GPIO_CLK_WP 	RCC_AHB1Periph_GPIOD
...
#define WP_L()        GPIO_ResetBits(GPIO_PORT_WP, GPIO_PIN_WP)
#define WP_H()        GPIO_SetBits(GPIO_PORT_WP, GPIO_PIN_WP)
...
void FRAM_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(GPIO_CLK_SDA | GPIO_CLK_CLK | GPIO_CLK_WP, ENABLE);
  ...
  GPIO_Init(GPIO_PORT_WP, &GPIO_InitStructure);
...
static uint8_t i2c_start(void)
{
  ...
  if(!SDA_READ()) return I2C_FALSE;  // SDA线为低电平则总线忙,退出
  SDA_L();
  i2c_delay(I2C_US_NUM);
  if(SDA_READ())  return I2C_FALSE;  // SDA线为高电平则总线出错,退出
  SCL_L();
  i2c_delay(I2C_US_NUM);
  return I2C_TRUE;
}
